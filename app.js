(() => {
  ...
  const nodes = {
    ...
    teamGrid: $("#team-grid"), teamCount: $("#team-count"), teamSearch: $("#team-search"), createTeam: $("#create-team"), mergeTeams: $("#merge-teams"), exportTeams: $("#export-teams"), importTeams: $("#import-teams"),
  ...
  function extractFirstArrayLiteral(text, marker) {
  ...
  function parseImportedTeamsSource(source) {
    ...
    try {
      ...
      if (Array.isArray(parsed?.INAZUMA_TEAMS)) return parsed.INAZUMA_TEAMS;
    } catch {}

    const arraySource = extractFirstArrayLiteral(text, "INAZUMA_TEAMS") || extractFirstArrayLiteral(text, "");
    if (!arraySource) throw new Error("Formato non riconosciuto. Importa un file esportato come teams.js o un JSON array.");

    try {
      const parsed = Function(`"use strict"; return (${arraySource});`)();
      if (!Array.isArray(parsed)) throw new Error("Il contenuto non è una lista di squadre.");
      return parsed;
    } catch (error) {
      throw new Error(`Impossibile leggere teams.js: ${error.message}`);
    }
  }
  ...
  async function importTeams(file) {
    ...
    try {
      ...
      const imported = parseImportedTeamsSource(source);
      if (!imported.length) throw new Error("Il file non contiene squadre.");

      const normalized = imported.map((team, index) => Store.normalizeTeam(team, `Imported Team ${index + 1}`));
      const hasNamedTeam = normalized.some((team) => team.name && team.name !== "Imported Team");
      if (!hasNamedTeam) throw new Error("Nessuna squadra valida trovata nel file.");

      const message = `Importare ${normalized.length} squadre?\n\nQuesta operazione sostituisce le squadre locali del Team Manager con quelle del file importato. Verrà creato un backup locale automatico.`;
      if (!window.confirm(message)) return;

      Store.backup(localStorage, teams, `Import teams.js (${normalized.length} teams)`);
      teams = Store.hydrate(officialPlayers, [], normalized);
      persist();
      ...
      window.alert(`Import squadre completato: ${teams.length} squadre caricate.`);
    } catch (error) {
      console.error("Team import failed", error);
      window.alert(`Import squadre fallito: ${error.message}`);
    } finally {
      if (nodes.importTeams) nodes.importTeams.value = "";
    }
  }
  ...
  nodes.teamSearch.addEventListener("input", () => { state.teamSearch = nodes.teamSearch.value; renderTeams(); }); nodes.createTeam.addEventListener("click", createTeam); nodes.mergeTeams.addEventListener("click", prepareMerge); nodes.exportTeams.addEventListener("click", exportTeams); nodes.importTeams?.addEventListener("change", () => importTeams(nodes.importTeams.files?.[0]));
