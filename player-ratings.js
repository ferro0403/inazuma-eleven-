...
L’operazione verrà sincronizzata su Firestore. Gli altri rating non verranno toccati.`);
  ...
  function saveRating(player, rating) {
    const id = playerId(player);
    const now = new Date().toISOString();
    ratings[id] = { ...normalizeRating(rating), playerId: id, autoGenerated: false, generatedFromOverall: Boolean(rating.generatedFromOverall), updatedAt: now, clientUpdatedAt: now, updatedBy: syncState.evaluatorName || "Utente" };
    persistRatings();
  ...
  function firestorePayload(id, rating) {
    ...
    const calculatedOverall = overallFor(playerById.get(String(id)), normalized);
    const payload = {
      playerId: String(id),
  ...
  function ratedPlayerPayload(player) {
    ...
    const effectiveRole = roleCode(player); const originalRole = originalRoleCode(player);
    return { playerId: playerId(player), id: playerId(player), name: player.name || "", portraitUrl: playerPortraitUrl(player), position: player.__ratingsTeamRole || player.position || player.role || "", originalPosition: player.__ratingsTeamRole ? originalRole : undefined, ratingRole: effectiveRole, element: player.element || playerElement(player), type: player.type || player.element || playerElement(player), teamIds: Array.isArray(player.teamIds) ? player.teamIds : [], teams: Array.isArray(player.teams) ? player.teams : [], custom: Boolean(player.custom), ...Object.fromEntries(STAT_DEFS.map(([stat]) => [stat, rating[stat]])), overall, category: categoryFor(overall) };
  }
