(() => {
  ...
  function enforceRoleRules(player, rawStats) {
    ...
    if (role === "FW") {
      ...
      stats.control = Math.max(5, stats.control);
      stats.attack = Math.max(stats.attack, stats.control, stats.speed - 1, stats.physical - 1, stats.grit - 1);
    }
    if (role === "MF") {
      stats.control = Math.max(5, stats.control);
      stats.stamina = Math.max(5, stats.stamina);
      stats.control = Math.max(stats.control, stats.stamina - 1, stats.grit - 1);
    }
    if (role === "DF") {
      stats.attack = Math.min(3, Math.max(1, stats.attack));
      stats.physical = Math.max(4, stats.physical);
      stats.grit = Math.max(4, stats.grit);
      stats.defense = Math.max(stats.defense, stats.physical, stats.grit, stats.speed - 1);
    }
    if (role === "GK") {
      stats.attack = Math.min(2, Math.max(1, stats.attack));
      stats.speed = Math.min(2, Math.max(1, stats.speed));
      stats.save = Math.max(stats.save, stats.physical + 1, stats.grit + 1, stats.defense + 1, stats.control, stats.stamina, stats.speed + 2, stats.attack + 2);
    }
    STAT_DEFS.forEach(([stat]) => { stats[stat] = clampStat(stats[stat]); });
    return stats;
  }
  ...
  function saveRating(player, rating) {
    ...
    persistRatings();
    updateSyncStatus("Salvato in localStorage. Usa il pulsante dedicato per caricare online.");
  }
  ...
  function importRatingsJson(file) {
    ...
    reader.onload = () => {
      try {
        ...
        persistRatings();
        render();
        updateSyncStatus(`Import locale completato: Rating importati: ${imported}. Rating ignorati: ${ignored}. Rating precedenti rimossi: ${previousCount}. Firestore: non modificato. Usa il pulsante dedicato per caricare online.`);
