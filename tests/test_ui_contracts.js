...
assert.doesNotMatch(index, /Carica rating locali su Firestore/);
assert.match(index, /sincronizza automaticamente ogni modifica con Firestore in tempo reale/);
...
assert.match(ratings, /firestoreConnected/);
assert.match(ratings, /syncEnabled/);
assert.match(ratings, /flushPendingSync/);
assert.match(ratings, /PENDING_SYNC_KEY/);
assert.match(ratings, /db\.batch\(\)/);
...
assert.match(ratings, /batch\.delete\(/);
assert.match(ratings, /Number\.isFinite\(numeric\)/);
assert.match(ratings, /function normalizeVariant/);
assert.match(ratings, /function validVariant/);
assert.match(ratings, /defaultRoleVariantId/);
assert.match(ratings, /roleSwitchEnabled/);
assert.match(ratings, /roleVariants/);
assert.match(ratings, /AGGIUNGI SECONDO RUOLO/);
assert.match(ratings, /IMPOSTA COME PREDEFINITA/);
assert.match(ratings, /ELIMINA VARIANTE/);
assert.match(ratings, /GRAFICA ALTERNATIVA/);
assert.match(ratings, /portraitUrl/);
assert.match(ratings, /frontFullbodyUrl/);
assert.match(ratings, /new Set\(roleVariants\.map/);
assert.match(ratings, /roleVariants\.length === 2/);
...
assert.doesNotMatch(ratings, /collection\.get\(\)/);
assert.match(ratings, /queueRatingSync\(id, "set"\)/);
assert.match(ratings, /Sync realtime attiva/);
